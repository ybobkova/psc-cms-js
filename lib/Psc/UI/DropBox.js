/*globals alert:true*/
define(['joose', 'jquery','jquery-ui', 'Psc/UI/DropBoxButton', 'Psc/UI/WidgetWrapper', 'Psc/CMS/FastItem', 'Psc/EventDispatching', 'Psc/Code', 'Psc/InvalidArgumentException', 'Psc/Exception'], function(Joose) {
  ...
  Joose.Class('Psc.UI.DropBox', {
    ...
    has: {
      ...
      removeEvent: { is : 'rw', required: false, isPrivate: true, init: true },
    ...
    methods: {
      initWidget: function () {
        ...
        if (this.$$removeEvent) {
          $dropBox.on('click', '.psc-cms-ui-button', function (e) {
        ...
        // remove appended buttons on dropbox.remove event
        $dropBox.on('dropbox.remove', '.psc-cms-ui-button', function (e) {
          var $button = $(e.currentTarget);
        ...
        this.getEventManager().on('drop-box-multiple-violated', function (e, button) {
          alert('Das Element befindet sich bereits in der Box. #'+button.getHash());
        });
      },
      /**
