...
define(['Psc/Exception','Psc/UI/Tab','Psc/Code','Psc/EventDispatching'], function() {
  /**
    ...
    methods: {
      ...
      initSortable: function () {
        ...
        if (this.$$sortable) {
          ...
          $table.find('td.ctrl').css('padding-left','1.5em').prepend(
            $('<span/>')
              .css('margin-left','-1.3em')
              .css('position', 'absolute')
              .addJoose.Class('ui-icon').addJoose.Class('ui-icon-arrowthick-2-n-s')
          ).each(function () {
            var $td = $(this), $cb = $td.find('input[type="checkbox"]');
          ...
          if ($tr.find('td.ctrl input.psc-cms-ui-identifier-checkbox:checked').length) {
            if (!toObject) {
              ...
              $tr.find('td').each(function (columnIndex, td) {
                var $td = $(td);
                if (!$td.hasJoose.Class('ctrl')) { // dieser filter muss nach innen damit columnIndex korrekt ist
