  ...
  Joose.Class('Psc.UI.GridTableEditor', {
    ...
    methods: {
      attach: function ($grid) {
        ...
        var columnNumber = that.$$grid.getColumn("correctOIDs").index;      
        
        that.$$grid.getHTMLRows().each(function(){
          var $tr = $(this);
          var rowNumber = that.$$grid.getHTMLRows().index($tr) + 1;
          $('tr:eq('+rowNumber+') td:eq('+columnNumber+')')
            .dblclick (function(event){
              var $table = $tr.closest('table');
              event.preventDefault();
              that.openChangeValueDialog(rowNumber, $table);
          });
        });
      ...
      openChangeValueDialog: function (rowNumber, $table) {
        var that = this;
        var $cell = that.$$grid.getCell(rowNumber,'correctOIDs');
        var dialog = this.$$dialog = new Psc.UI.Dialog({
          title: 'Daten der Zelle bearbeiten',
          onSubmit: function(e, dialog){
            e.preventDefault();
            var newOIDs = $('input:text').val();
            newOIDs = newOIDs.split(", ");
            var oidArray = [];
            $.each(newOIDs, function(){
              var oid = $(this);
              oidArray.push(eval(oid[0]));
            });
            that.$$grid.setCell(rowNumber, 'correctOIDs', oidArray);
            var rows = that.$$grid.unwrap();
            that.$$grid.refresh();
            dialog.close();
            $cell = that.$$grid.getCell(rowNumber,'correctOIDs');
            console.log($cell);
          }
        });
      ...
      _createChangeForm: function($cell) {
       var $form =
          $('<form>')
            .append('<input type="text" name="oids" value="' + $cell + '" autofocus /><br />')
            .append('<small class="hint">Hier die OIDs eingeben (getrennt mit einem Komma oder Leerzeichen)<br /></small>');
