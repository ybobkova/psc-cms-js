  ...
  Joose.Class('Psc.UI.GridTableEditor', {
    ...
    methods: {
        ...
        that.$$grid.getHTMLRows().each(function () {
          ...
          $('tr:eq('+rowNumber+') td:eq('+columnNumber+')')
            .dblclick (function(event){
              ...
              that.$$grid.onCellsDoubleClick(function (event, $table, rowNumber) {
      ...
      openChangeValueDialog: function (rowNumber, $table) {
        ...
        var dialog = this.$$dialog = new Psc.UI.Dialog({
          ...
          onSubmit: function(e, dialog){
            ...
            newOIDs = newOIDs.replace(/\s/g, '');
            newOIDs = newOIDs.split(/,/);
            var oidArray = [];
            $.each(newOIDs, function(i, oid){
              if (oid.match(/^[0-9]+$/)){ 
                oid = parseInt(oid, 10);
              }
              oidArray.push(oid);
      ...
      _createChangeForm: function(cell) {
       var $form =
          $('<form>')
            .append('<input type="text" name="oids" class="inputValue" value="' + cell + '" autofocus /><br />')
            .append('<small class="hint">Hier die OIDs eingeben (mit Komma getrennt)<br /></small>');
       return $form;
      },
