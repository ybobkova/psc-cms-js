    ...
    methods: {
      attach: function ($grid) {
        ...
        that.$$grid.getHTMLRows().each(function () {
          var $tr = $(this);
          var rowIndex = that.$$grid.getHTMLRows().index($tr) + 1;
          $('tr:eq('+rowIndex+') td')
            .dblclick (function(event){
              var $cell = $(event.currentTarget);
              var columnIndex = that.$$grid.getColumnIndex($cell);
      ...
      openChangeValueDialog: function (rowIndex, columnIndex, $table) {
        var that = this;
        ...
        var dialog = this.$$dialog = new Psc.UI.Dialog({
          ...
          onSubmit: function(e, dialog){
            e.preventDefault();
            var newCell = $('input:text').val();
            if (columnType === "Array") {
              newCell = newCell.split(/,/);
              var elementsArray = [];
              $.each(newCell, function(i, element){
              ...
              that.$$grid.setCell(rowIndex, columnName, newCell);
            }
            dialog.close();
            that.$$grid.refresh();
          }
