  ...
  Joose.Class('Psc.UI.GridTableEditor', {
    ...
    methods: {
      ...
      _createChangeForm: function(cell) {
        ...
        var $form =
          $('<form>')
            .append('<textarea name="text" class="inputValue" cols="75" rows="1">' + cell + '</textarea>')
            .append('<small class="hint">Hier den neuen Inhalt der Zelle eingeben.<br />OIDs - durch Komma getrennt.<br /></small>');

        var textArea = $form.find('textarea.inputValue');
        
        textArea.bind("keypress", function() {
          var newRows = Math.ceil(this.value.length / this.cols);
          this.rows = newRows;
        });
