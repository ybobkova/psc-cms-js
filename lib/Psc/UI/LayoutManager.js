Joose.Class('Psc.UI.LayoutManager', {
  ...
  methods: {
    initUI: function () {
      ...
      this.$$layout.on('keyup', function (e) {
        if (e.keyCode === $.ui.keyCode.ENTER) {
          ...
          if (match != null) {
            var $widget = $ta.closest("div.widget");
    ...
    createWidget: function (type, content) {
      ...
      if (type === 'paragraph') {
        widget = new Psc.UI.LayoutManager.Paragraph({label: 'Absatz', content: content});
      } else if (type === 'headline') {
        widget = new Psc.UI.LayoutManager.Headline({label: 'Überschrift', level: 1, content: content});
      } else if (type === 'sub-headline') {
        widget = new Psc.UI.LayoutManager.Headline({label: 'Zwischenüberschrift', level: 2, content: content});
      } else if (type === 'list' || type === 'li') {
        widget = new Psc.UI.LayoutManager.Li({label: 'Aufzählung', content: content});
      } else if (type === 'websitewidget') {
        widget = new Psc.UI.LayoutManager.Websitewidget({label: content.label, name: content.name, content: content});
      } else if (type === 'downloadslist') {
        widget = new Psc.UI.LayoutManager.Downloadslist({
          label: 'Downloads-Liste',
