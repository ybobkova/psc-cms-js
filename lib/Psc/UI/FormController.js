define(
  ...
  Joose.Class('Psc.UI.FormController', {
    ...
    methods: {
       ...
       */
      save: function (tabHook, revision) {
        ...
        this.$$ajaxFormHandler.handle(formRequest)
           .done(function (response) {
             evm.triggerEvent(
               'form-saved', {}, [that.getForm(), response, tabHook, revision]
             );
           })
           .fail(function (response) {
            if (Psc.Code.isInstanceOf(response, Psc.Response)) {
             evm.triggerEvent(
               'error-form-save', {}, [that.getForm(), response, revision]
             );
            } else {
             evm.triggerEvent(
               'error-form-exception', {}, [response, formRequest, that.getForm(), revision]
             );
            }
           });
      },
    ...
    my: {
      methods: {
        changeRevision: function(newRevision, $form) {
          if ($form.jquery) {
            var $headerField = $form.find('input[name="X-Psc-Cms-Revision"]');
  
            if ($headerField.length) {
              $headerField.val(newRevision);
            }
         ...
         * @return string|false
         */
        readRevision: function($form) {
          if ($form.jquery) {
            var $headerField = $form.find('input[name="X-Psc-Cms-Revision"]');
          
            if ($headerField.length) {
              return $headerField.val();
            }
          }
