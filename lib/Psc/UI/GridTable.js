define(['joose', 'Psc/TableModel', 'Psc/Code', 'Psc/Exception', 'Psc/InvalidArgumentException','Psc/UI/WidgetWrapper', 'Psc/UI/HTML/Base'], function(Joose) {
  ...
  Joose.Class('Psc.UI.GridTable', {
    ...
    has: {
      ...
      onCellsDoubleClickCallbacks: { is: 'r', required: false, isPrivate: true, init: []}
    },
    ...
    methods: {
      ...
      refresh: function () { // wird jedes mal von base aufgerufen, wenn html() aufgerufen wird
        ...
        if (this.widget) {
          ...
          var columnNum = that.getColumn("correctOIDs").index + 1;
          this.widget.on('dblclick', 'tr td:nth-child('+columnNum+')', function (event) {
            var $cell = $(event.currentTarget);
            var $tr = $cell.parent();
            var $table = $tr.closest('table');
            var $trs = that.widget.find('tr');
            var rowNum = $trs.index($tr);
            for (var i = 0; i < that.$$onCellsDoubleClickCallbacks.length; i++) {
              that.$$onCellsDoubleClickCallbacks[i](event, $table, rowNum);
            }
          });
        }
