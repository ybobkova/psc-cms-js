define(['joose', 'jquery', 'Psc/UI/Menu','Psc/InvalidArgumentException', 'Psc/Exception', 'Psc/Code'], function(Joose, $) {
  Joose.Class('Psc.UI.ContextMenuManager', {
    
    has: {
      idStorageKey: { required: false, isPrivate: true, init: 'context-menu-manager-id' },
      menus: { required: false, isPrivate: true, init: Joose.I.Object },
      menuIndex: { required: false, isPrivate: true, init: 0 },
    ...
    methods: {
      register: function ($owner, menu) {
        ...
        $menu.addClass('psc-cms-ui-context-menu');
        
        // lets store the index for the menu inside the owner
        var index = this.index(owner);
        if (!$.isNumeric(index)) {
          $.data(owner, this.$$idStorageKey, index = this.newIndex());
          menu.setOwner($owner);
        }
