...
define(['joose', 'Psc/UI/Dialog', 'Psc/UI/FormBuilding', 'Psc/UI/FormReading', 'Psc/UI/Button', 'Psc/UI/Controlling', 'Psc/UI/Translating'], function(Joose) {
  Joose.Class('Psc.UI.NavigationNode', {
    ...
    methods: {
      html: function () {
        if (!this.$$li) {
          this.$$li = $('<li class="dd-item"></li>');
          this.$$li.data('node', this);

          var $handle = $('<div class="dd-handle ui-state-default ui-corner-all"></div>');
          ...
          $buttonPanel
            .addClass('dd-nodrag') // say nestable to discard events for dragging from this container
            .append(this.htmlEditButton())
            .append(this.htmlDeleteButton())
            .appendTo($handle);

          $handle.append('<span class="title">'+this.getTitle()+'</span>');
          $handle.appendTo(this.$$li);
        }
      ...
      refreshTitle: function () {
        if (this.$$li.length) {
          this.$$li.find('.title').html(this.getTitle());
        }
      },
