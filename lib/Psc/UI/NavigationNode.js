...
define(['Psc/UI/Dialog', 'Psc/UI/FormBuilding', 'Psc/UI/FormReading', 'Psc/UI/Button'], function() {
  Joose.Class('Psc.UI.NavigationNode', {
    
    does: [Psc.UI.FormBuilding, Psc.UI.FormReading, Psc.UI.Button],
    
    has: {
      id: { is : 'rw', required: false, isPrivate: true },
      title: { is : 'rw', required: true, isPrivate: true },
      level: { is : 'rw', required: false, isPrivate: true, init: 0 },
    ...
    methods: {
      ...
      html: function () {
        if (!this.$$li) {
          ...
          //if (this.$$pageId <= 0 && this.$$id > 0) { // nicht neu, aber ohne page
            //this.$$li.append(
            //  $('<span class="no-page ui-icon ui-icon-close" title="Der Navigations-Punkt ist noch mit keiner Seite verknüpft"></span>')
      ...
      htmlPageButton: function () {
        ...
        return button.create().attr('title','Der Navigations-Punkt hat eine verknüpfte Seite.');
      },
      ...
      htmlNoPageButton: function () {
        ...
        return button.create().attr('title','Der Navigations-Punkt hat noch keine verknüpfte Seite!');
      },
      ...
      setPageId: function (id) {
        //if (id > 0 && this.$$li) {
          //var $noPage = this.$$li.find('.no-page');
          //if ($noPage.length) $noPage.remove();
        //}
        this.$$pageId = id;
      },
      ...
      refresh: function () {
        this.html().css('margin-left',3+this.$$level*this.getColWidth());
      },
