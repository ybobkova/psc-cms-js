  ...
  Joose.Class('Psc.UI.NavigationNode', {
    ...
    methods: {
      ...
      htmlEditButton: function() {
        var $button = $('<button class="psc-cms-ui-button edit ui-button ui-widget ui-state-default ui-button-text-icon-primary" role="button" aria-disabled="false"><span class="ui-button-icon-primary ui-icon ui-icon-pencil"></span><span class="ui-button-text">edit</span></button>');
        
        return $button;
      },
      htmlDeleteButton: function() {
        var $button = $('<button class="psc-cms-ui-button delete ui-button ui-widget ui-state-default ui-button-text-icon-primary ui-corner-right" ><span class="ui-button-icon-primary ui-icon ui-icon-trash"></span><span class="ui-button-text">del</span></button>');
        
        return $button;
      },
      
      htmlPageButton: function () {
        var $button = $('<button class="open-page psc-cms-ui-button ui-button ui-widget ui-state-default ui-button-text-icon-primary ui-corner-left" role="button" aria-disabled="false" title="Der Navigations-Punkt hat eine verknüpfte Seite."><span class="ui-button-icon-primary ui-icon ui-icon-script"></span><span class="ui-button-text">Seite öffnen</span></button>');
        ...
        if (this.$$id > 0) {
          for (i=0; i<this.$$languages.length; i++) {
            ...
            tabButtonItem = ui.createTabButtonItem(
              ui.tab('navigation-node', this.$$id, ['contentstream', 'de'], "Navigationsinhalt: "+this.getTitle(lang)+" ("+lang.toUpperCase()+")"),
              ui.button("Inhalt bearbeiten ("+lang.toUpperCase()+")", 1)
            );
