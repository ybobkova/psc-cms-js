define(['jquery', 'joose', 'Psc/UI/WidgetWrapper', 'Psc/Exception', 'jquerypp/dom/selection'], function ($, Joose) {
  ...
  Joose.Class('Psc.TextEditor', {
    ...
    methods: {
      move: function(caretPosition) {
        this.unwrap().selection(caretPosition, caretPosition);
      },
      getCaret: function () {
        // this works in ie. getSelection() does not
        var caret = this.unwrap().prop("selectionStart");
        
        if (this.unwrap().prop("selectionStart") !== this.unwrap().prop("selectionEnd")) {
          return false;
        }
      ...
      insertSpaced: function(insertText, position) {
        ...
        // pad left?
        if (insertText.substr(0,1) !== ' ' && position > 0 && this.lookbehind(position, 1) !== ' ') {
        ...
        return this.insert(insertText, position);
      },
