...
Joose.Class('Psc.Date', {
  ...
  use: [
    $.psc.getUsePresence('jquery-ui'), $.psc.getUsePresence('jquery-ui-i18n'), 'Psc.Code', 'Psc.Exception'
  ],
  ...
  my: {
    methods: {
      ...
      create: function (props) {
        ...
        if (props.day) {
          return new Psc.Date({
            date: new Date(props.year, props.month-1, props.day, props.hours || 0, props.minutes || 0, props.seconds || 0)
          });
        }
  ...
  methods: {
    ...
    toString: function() {
      return "[Psc.Date "+this.format('$d.m.yy')+"]";
    },
