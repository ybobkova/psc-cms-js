...
define(['joose'], function(Joose) {
  Joose.Class('Psc.TextParser', {
    ...
    methods: {
      ...
      parse:  function (text) {
        var l = 0, arrayList = [], parsed = [], listFlag = false,
            tokens = this.tokenize(text);
						 // nextToken = tokens[l+1];

        while (l < tokens.length) {
          if (tokens[l].type === "listpoint") {
            if (!listFlag) {
              arrayList.push(tokens[l].value);
              listFlag = true;
            } else {
              arrayList.push(tokens[l].value);
              if ((l < tokens.length-1 && tokens[l+1].type === "doublebreak") || (l === tokens.length - 1)){
            ...
            if (tokens[l].type !== "doublebreak") {
              if (listFlag) {
                parsed.push({type:"list", value:arrayList});
                arrayList = [];
                listFlag = false;
                parsed.push({type:"paragraph", value:tokens[l].value});
              } else {
                parsed.push({type:"paragraph", value:tokens[l].value});
              }
            }
