Joose.Class('tiptoi.GameTester', {
  ...
  use: ['Psc.Code',
        'tiptoi.SimpleSoundPlayer','tiptoi.ProgramRunner','tiptoi.Program','tiptoi.InputProvider','tiptoi.Sound','tiptoi.ConsoleOutput'],
  ...
  methods: {
    run: function() {
      ...
      status.progress(function (cpu) {
        ...
        var output = new tiptoi.ConsoleOutput({eventManager: cpu.getEventManager()});
      });
      
      status.fail(function (error) {
        Psc.Code.warning(error);
        alert('Game '+program.getName()+' kann nicht ausgeführt werden: '+error);
      });
